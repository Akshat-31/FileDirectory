...
Server 2-> responde with OK and creates the file at the given Path
Server 1->Sends the content of file
Server 2-> Reads the content and writes on file and respons to server1
*/
int saveLocally(int con_fd, long fileSize, char *fileName, char *destinationPath){
    //Creates a local path to store file
    char *user=getenv("USER");
    char fixedPath[1026];
    snprintf(fixedPath, sizeof(fixedPath),"/Users/%s/Downloads/S2",user);
    char tempPath[1026];
...
Server2-> Creates a local path and downloads and responde with OK fileSize
Server1->Sends the confirmation
Server2-> Sends the file content
*/
...
Server2-> Creates a local path and removes the file and sends the response to client
*/
...
Server2-> Creates a temp file to store all filtered file. Creates tar from reading this file and sends OK file Size to client
Server1->Sends the confirmation
Server2-> Sends the file content
*/
void createTARfilesLocally(int con_fd){
    ...
    snprintf(temp_tar_path, sizeof(temp_tar_path),"/Users/%s/Downloads/temp_tar_02.tar",user);
    //Creating path to be searched
    char search_path[1026];
    snprintf(search_path, sizeof(search_path),"/Users/%s/Downloads/S2",user);
    //Creating temp file to store filtered results
    char file_found_path[1026];
    snprintf(file_found_path, sizeof(file_found_path),"/Users/%s/Downloads/files_found_02.tar",user);
    //Creating a find command
    char find_command[1026];
    snprintf(find_command,sizeof(find_command),"find \"%s\" -name \"*.pdf\" -type f > \"%s\" ",search_path,file_found_path);
    system(find_command);
